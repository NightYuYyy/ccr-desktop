...
export function registerConfigHandlers() {
  // 读取设置文件的处理器
  MainIPCService.handle('read-settings', async () => {
  ...
  // 打开配置文件夹的处理器
  MainIPCService.handle('open-config-folder', async () => {
    const paths = ConfigManager.getCCRPaths()
    console.log('[ConfigHandler] 尝试打开配置文件夹:', paths.configDir)

    await require('electron').shell.openPath(paths.configDir)
  ...
  // 打开Claude配置文件夹的处理器
  MainIPCService.handle('open-claude-config-folder', async () => {
    const claudeConfigDir = ConfigManager.getClaudeConfigDir()
    console.log('[ConfigHandler] 尝试打开Claude配置文件夹:', claudeConfigDir)

    await require('electron').shell.openPath(claudeConfigDir)
  ...
  // 保存配置文件的处理器
  MainIPCService.handle('save-settings', async (event, configData) => {
  ...
  // 执行命令的处理器（支持实时输出）
  MainIPCService.handle('exec-command', async (event, command) => {
  ...
  // 添加Provider的处理器
  MainIPCService.handle('add-provider', async (event, providerData) => {
  ...
  // 更新Provider的处理器
  MainIPCService.handle('update-provider', async (event, providerName, updatedData) => {
  ...
  // 删除Provider的处理器
  MainIPCService.handle('delete-provider', async (event, providerName) => {
  ...
  // 更新默认模型的处理器
  MainIPCService.handle('update-default-model', async (event, defaultModel) => {
  ...
  // 更新路由器模型的处理器
  MainIPCService.handle('update-router-model', async (event, modelType, modelValue) => {
  ...
  // 更新长文本阈值的处理器
  MainIPCService.handle('update-long-context-threshold', async (event, threshold) => {
  ...
  // 读取文件的处理器
  MainIPCService.handle('read-file', async (event, filePath) => {
    const result = await readJsonFile(filePath)
    return result
  })

  // 写入文件的处理器
  MainIPCService.handle('write-file', async (event, filePath, content) => {
  ...
  // 检测网络模式的处理器
  MainIPCService.handle('detect-network-mode', async () => {
    try {
      console.log('[ConfigHandler] 开始检测网络模式')

      // 使用统一的FloatingService检测网络模式
      const networkMode = await FloatingService.detectNetworkMode()

      console.log('[ConfigHandler] 网络模式检测结果:', networkMode.isProxy ? '代理' : '直连')

      return {
        success: true,
        isProxy: networkMode.isProxy,
        mode: networkMode.mode,
        message: networkMode.isProxy
          ? '检测到CCR服务，使用代理模式'
          : '未检测到CCR服务，使用直连模式'
      }
    } catch (error) {
  ...
  // 读取直连配置文件
  MainIPCService.handle('read-direct-config', async () => {
  ...
  // 保存直连配置文件
  MainIPCService.handle('save-direct-config', async (event, configData) => {
  ...
  MainIPCService.handle('apply-direct-config', async (event, directConfig) => {
  ...
  // 切换网络模式处理器
  MainIPCService.handle('switch-network-mode', async (event, isProxy) => {
...
function refreshFloatingWindow() {
  setImmediate(() => {
    require('electron').ipcMain.emit('refresh-floating-window')
  })
}
 ...
 * @param {Function} operationFn - 执行函数
...
export function unregisterConfigHandlers() {
  MainIPCService.removeHandler('read-settings')
  MainIPCService.removeHandler('get-config-paths')
  MainIPCService.removeHandler('open-config-folder')
  MainIPCService.removeHandler('open-claude-config-folder')
  MainIPCService.removeHandler('save-settings')
  MainIPCService.removeHandler('exec-command')
  MainIPCService.removeHandler('add-provider')
  MainIPCService.removeHandler('update-provider')
  MainIPCService.removeHandler('delete-provider')
  MainIPCService.removeHandler('update-default-model')
  MainIPCService.removeHandler('update-router-model')
  MainIPCService.removeHandler('update-long-context-threshold')
  MainIPCService.removeHandler('get-home-dir')
  MainIPCService.removeHandler('get-claude-settings-path')
  MainIPCService.removeHandler('read-file')
  MainIPCService.removeHandler('write-file')
  MainIPCService.removeHandler('detect-network-mode')
  MainIPCService.removeHandler('get-direct-config-path')
  MainIPCService.removeHandler('read-direct-config')
  MainIPCService.removeHandler('save-direct-config')
  MainIPCService.removeHandler('apply-direct-config')
  MainIPCService.removeHandler('switch-network-mode')
  MainIPCService.removeHandler('set-webdav-config')
  MainIPCService.removeHandler('get-webdav-config')
  MainIPCService.removeHandler('test-webdav-connection')
  MainIPCService.removeHandler('backup-data-webdav')
  MainIPCService.removeHandler('list-webdav-backups')
